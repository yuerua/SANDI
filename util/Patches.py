...
class Patches:
    ...
    def merge_patches(self, image, patches):
        img_patch_h = self.img_patch_h
        img_patch_w = self.img_patch_w
        label_patch_h = self.label_patch_h
        label_patch_w = self.label_patch_w
...
if __name__ == '__main__':
    ...
    cell_patch = np.random.rand(patch_size,patch_size,3)
    split_patch = patch_obj.extract_patches(cell_patch)
    cell_patch_stitch = patch_obj.merge_patches(np.zeros(patch_size,patch_size,3), split_patch)